...
function map() {
    ...
    global.app.get('/api/automovel/buscarClienteId/:id', global.checkAuth([1, 2, 3, 4]), function(req, res) {
        var id = req.params.id;
        global.db.query(global.services.automovel.findByClientId(id), function(err, rows) {
            global.error(err);
            res.send(rows);
        });
    });
